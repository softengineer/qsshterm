...
void QTermWidget::keyPressEvent( QKeyEvent* event ) {
    QString msg = event->text();
    QByteArray data = msg.toLatin1();
    m_impl->m_session->onReceiveBlock(data.data(), data.length());
}

void QTermWidget::receivedRemoteData(QByteArray data) {
    emit m_impl->m_session->onReceivedRemoteData(data);
}
...
void QTermWidget::init(int startnow)
{
    ...
    connect(m_impl->m_terminalDisplay, &TerminalDisplay::configureRequest,  this, &QTermWidget::mouseEventEmiter);
}

void QTermWidget::mouseEventEmiter() 
{
